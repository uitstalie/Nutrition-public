...
/**
 * DataGen Provider：生成 neotrition group JSON。
 * 输出到 {@code data/neotrition/groups/}。
 */
public class NutritionGroupProvider extends JsonCodecProvider<NutritionGroupJson> {
    ...
    public NutritionGroupProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider, ExistingFileHelper existingFileHelper) {
        super(output, PackOutput.Target.DATA_PACK, "groups", PackType.SERVER_DATA, NutritionGroupJson.CODEC, lookupProvider, Neotrition.MOD_ID, existingFileHelper);
    }
    ...
    private void add(String name, String icon, int color) {
        this.unconditional(
                ResourceLocation.fromNamespaceAndPath(Neotrition.MOD_ID, name),
